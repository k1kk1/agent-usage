...
| 項目 | 内容 |
| --- | --- |
| 今日のトークン | JST の当日ぶん。日別推移のスパークラインとコスト概算を併記 |
| セッションのトークン | 直近のセッションぶん |

既定はアプリが「今日」と「セッション」、ウィジェットが「今日」、メニューバーは横幅が
貴重なため両方オフです。メニューバーで有効にすると `12% / 1.2M` のように利用率と並びます。

集計は `agent-status-daemon.sh` が state.json に書いた値をそのまま読むだけで、アプリ側は
ログを読みません。トークンはローカルのログ由来なので、利用枠の取得に失敗していても
表示が残ります。
